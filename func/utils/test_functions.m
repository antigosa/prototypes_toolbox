...
%% test 16 - save data (two conditions - different prototypes locations)
clear;
close all;
ndots_x                 = 20;
ndots_y                 = 10;
...
ActualDots_xy           = prototypes_generate_grid('Rectangle', ShapeDim, ndots_x, ndots_y, multOf, grid_offset, dot_noise);
SubjectsData            = prototypes_synthetic_simpleDS(ActualDots_xy, ShapeDim, 0, nsubj);
SubjectsData.Properties.UserData.StimulusType = 'Rectangle';
...
SubjectsDataA.CategoryID = [];
SubjectsDataA.CategoryPrototypes = [];

figure; prototypes_plot_dots(SubjectsDataA)
...
% set the options for the CAM model
opt.w                   = 0.75;
opt.prototypes          = [0.35 0.25; 0.35 0.75; 0.85 0.25; 0.85 0.75].*ShapeDim;
opt.method              = 'CategoryPrototypes';
...
SubjectsDataB.CategoryID = [];
SubjectsDataB.CategoryPrototypes = [];
