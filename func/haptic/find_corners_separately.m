function corners = find_corners_separately(I, threshold, offset, grayImage, ROI, plotImages)
...
if length(size(I))==3
    ...
    mask_all = zeros(size(I,1), size(I, 2), 4);
    mask = struct;
    % for each point
    for j = 1:4
        
        mask(j).mask = zeros(size(I,1), size(I, 2), size(I,3));
        for i=1:3
            mask(j).mask(:,:,i) = Is(:,:,i)>=threshold(j).(cols{i})(1) & Is(:,:,i)<=threshold(j).(cols{i})(2);
        end
        
        mask_all(:,:,j) = all(mask(j).mask, 3);
    end
    
    mask_all = any(mask_all, 3);
    Is = im2gray(Is);
else
    mask_all = Is>=threshold;
end
% figure;imshow(mask(1).mask);
% figure;imshow(mask(2).mask);
% figure;imshow(mask(3).mask);
% figure;imshow(mask(4).mask);
% figure;imshow(mask_all);

% 2. Label the connected components
% This assigns a unique ID to each blob (1, 2, 3, 4...)
[labeledImg, numBlobs] = bwlabel(mask_all);
