...
switch method
    ...
    case 'SST_mag'
        [R2, C] = prototypes_R2_SST_mag(ActualDots_xy, RespDots_xy, Predicted_xy, doPlot);        
        
    case 'mvarCorr'
        [R2, C, SST, SSR, SSM] = prototypes_R2_mvarCorr(ActualDots_xy, RespDots_xy, Predicted_xy, doPlot);
        
    case 'euclideanDist'
        [R2, C] = prototypes_R2_mvarEuclidean(ActualDots_xy, RespDots_xy, Predicted_xy, doPlot);
end
...
function [R2, C] = prototypes_R2_SST_mag(ActualDots_xy, RespDots_xy, Predicted_xy, doPlot)
% =========================================================================
% Compute R2
% =========================================================================

true_x = ActualDots_xy(:, 1);
true_y = ActualDots_xy(:, 2);

human_x = RespDots_xy(:,1);
human_y = RespDots_xy(:,2);

model_x = Predicted_xy(:,1);
model_y = Predicted_xy(:,2);


% 2. Calculate error magnitude (Euclidean distance from the true dot)
obs_error_mag  = sqrt((human_x - true_x).^2 + (human_y - true_y).^2);
model_error_mag = sqrt((model_x - true_x).^2 + (model_y - true_y).^2);

% % 3. Calculate Total Sum of Squares based on human error variance
% % (How much do human error magnitudes vary around the average human error?)
% SST = sum((obs_error_mag - mean(obs_error_mag)).^2);
% 
% % 4. Calculate Residual Sum of Squares 
% % (Squared difference between model error magnitude and human error magnitude)
% SSR = sum((model_error_mag - obs_error_mag).^2);
% 
% % compute the model sum of squares
% SSM = SST - SSR;
% 
...
R2 = 1-pdist([obs_error_mag'; model_error_mag'],'corr');
...
R2 = pdist([errorsAR(:)'; errorsAP(:)'],'euclidean');
